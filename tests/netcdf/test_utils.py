...
class TestSafeArrayNames:
    ...
    def test_returns_empty_list_on_none(self):
        ...
        result = _safe_array_names(grp)
        assert result == [], (
            "Should return empty list when None"
        )
...
class TestGetRootGroup:
    ...
    def test_returns_root_group(self):
        ...
        assert result is sentinel, (
            "Should return the root group object"
        )
...
class TestGetDriverName:
    ...
    def test_returns_driver_short_name(self):
        ...
        assert result == "netCDF", (
            "Should return the driver short name"
        )
...
class TestExportSrs:
    ...
    def test_returns_none_tuple_for_none_input(self):
        ...
        assert projjson is None, (
            "PROJJSON should be None for None SRS"
        )
    ...
    def test_exports_both_wkt_and_projjson(self):
        ...
        assert wkt == "GEOGCS[...]", (
            "WKT should match ExportToWkt result"
        )
    ...
    def test_wkt_fails_projjson_succeeds(self):
        ...
        wkt, projjson = _export_srs(srs)
        assert wkt is None, (
            "WKT should be None when ExportToWkt fails"
        )
        assert projjson == '{"type":"GeographicCRS"}', (
            "PROJJSON should still succeed"
        )
...
class TestGetArrayNodata:
    ...
    def test_fill_value_from_attrs(self):
        ...
        assert result == -9999.0, (
            "Should use _FillValue from attributes"
        )
    ...
    def test_missing_value_from_attrs(self):
        ...
        result = _get_array_nodata(mdarr, attrs)
        assert result == -1, (
            "Should fall back to missing_value"
        )
    ...
    def test_fill_value_empty_list_returns_none(self):
        ...
        result = _get_array_nodata(mdarr, attrs)
        assert result is None, (
            "Empty list _FillValue should return None"
        )
    ...
    def test_gdal_api_nodata_as_double(self):
        ...
        result = _get_array_nodata(mdarr, attrs)
        assert result == -999.0, (
            "Should fall back to GDAL API double"
        )
    ...
    def test_gdal_api_returns_tuple_with_flag(self):
        ...
        result = _get_array_nodata(mdarr, attrs)
        assert result == -9999.0, (
            "Should unpack (value, True) tuple"
        )
    ...
    def test_gdal_api_returns_tuple_with_false_flag(self):
        ...
        result = _get_array_nodata(mdarr, attrs)
        assert result is None, (
            "Should return None when GDAL flag is False"
        )
    ...
    def test_gdal_api_all_methods_fail(self):
        ...
        result = _get_array_nodata(mdarr, attrs)
        assert result is None, (
            "Should return None when all API methods fail"
        )
    ...
    def test_returns_none_when_no_attrs_no_api(self):
        ...
        result = _get_array_nodata(mdarr, attrs)
        assert result is None, (
            "Should return None with no info available"
        )
    ...
    def test_fill_value_string_attr(self):
        ...
        result = _get_array_nodata(mdarr, attrs)
        assert result == "N/A", (
            "Should return string _FillValue"
        )
    ...
    def test_fill_value_takes_precedence_over_missing_value(self):
        ...
        result = _get_array_nodata(mdarr, attrs)
        assert result == -1, (
            "_FillValue should have higher precedence"
        )
...
class TestGetArrayScaleOffset:
    ...
    def test_gdal_api_raises_keeps_attr_value(self):
        ...
        assert scale == pytest.approx(2.0), (
            "Should keep CF scale when GDAL API fails"
        )
        assert offset == pytest.approx(5.0), (
            "Should keep CF offset when GDAL API fails"
        )
    ...
    def test_integer_attrs_converted_to_float(self):
        ...
        assert isinstance(offset, float), (
            "Integer offset should be converted to float"
        )
    ...
    def test_non_numeric_attrs_ignored(self):
        ...
        assert scale is None, (
            "Non-numeric scale_factor should be ignored"
        )
        assert offset is None, (
            "Non-numeric add_offset should be ignored"
        )
...
class TestGetBlockSize:
    ...
    def test_returns_block_size(self):
        ...
        assert result == [256, 256], (
            "Should return block sizes as list of ints"
        )
    ...
    def test_returns_none_on_empty(self):
        ...
        result = _get_block_size(mdarr)
        assert result is None, (
            "Should return None for empty block size"
        )
    ...
    def test_returns_none_on_none_value(self):
        ...
        result = _get_block_size(mdarr)
        assert result is None, (
            "Should return None when GetBlockSize returns None"
        )
    ...
    def test_converts_elements_to_int(self):
        ...
        result = _get_block_size(mdarr)
        assert result == [64, 128, 32], (
            "Should convert elements to int"
        )
...
class TestGetCoordVariableNames:
    ...
    def test_falls_back_to_str(self):
        ...
        result = _get_coord_variable_names(mdarr)
        assert len(result) == 1, (
            "Should return one name via str() fallback"
        )
    ...
    def test_get_full_name_raises_falls_to_str(self):
        ...
        result = _get_coord_variable_names(mdarr)
        assert len(result) == 1, (
            "Should still produce a name via str() fallback"
        )
...
class TestNormalizeOriginString:
    ...
    def test_iso_t_separator(self):
        ...
        assert result == "1979-01-01 00:00:00", (
            "T separator should be replaced"
        )

    def test_date_only(self):
        """Date-only origin gets midnight time."""
        result = _normalize_origin_string("2000-6-15")
        assert result == "2000-06-15 00:00:00", (
            "Date-only should append 00:00:00"
        )

    def test_already_padded(self):
        """Already zero-padded input is unchanged."""
        result = _normalize_origin_string(
            "1979-01-01 00:00:00"
        )
        assert result == "1979-01-01 00:00:00", (
    ...
    def test_year_month_only(self):
        ...
        assert result == "2000-06-01 00:00:00", (
...
class TestParseUnitsOrigin:
    ...
    def test_days_since(self):
        ...
        assert origin == datetime(1979, 1, 1), (
            "Origin should be 1979-01-01"
        )
    ...
    def test_hours_since(self):
        ...
        assert origin == datetime(2000, 1, 1), (
            "Origin should be 2000-01-01"
        )
    ...
    def test_seconds_since(self):
        ...
        assert origin == datetime(1970, 1, 1), (
            "Origin should be Unix epoch"
        )
...
class TestCreateTimeConversionFunc:
    ...
    def test_days_conversion(self):
        ...
        assert convert(0) == "1979-01-01 00:00:00", (
            "Day 0 should be origin"
        )
        assert convert(365) == "1980-01-01 00:00:00", (
            "Day 365 should be next year"
        )
    ...
    def test_hours_conversion(self):
        ...
        assert convert(24) == "2000-01-02 00:00:00", (
            "24 hours should be next day"
        )
    ...
    def test_minutes_conversion(self):
        ...
        assert convert(60) == "2000-01-01 01:00:00", (
    ...
    def test_seconds_conversion(self):
        ...
        assert convert(3600) == "2000-01-01 01:00:00", (
            "3600 seconds should be 1 hour"
        )
    ...
    def test_fractional_days(self):
        ...
        assert convert(0.5) == "2000-01-01 12:00:00", (
            "0.5 days should be noon"
        )

    def test_negative_offset(self):
        """Negative offset goes before origin."""
        convert = create_time_conversion_func(
            "days since 2000-01-02"
        )
...
class TestDtypeToStr:
    ...
    def test_returns_name_from_get_name(self):
        ...
        assert result == "Float64", (
            "Should return the name from GetName"
        )
    ...
    def test_returns_unknown_when_all_fail(self):
        ...
        dt.__str__ = MagicMock(
            side_effect=RuntimeError("also fail")
        )
        result = _dtype_to_str(dt)
        assert result == "unknown", (
            "Should return 'unknown' as last resort"
        )
...
class TestToPyScalar:
    ...
    def test_numpy_int(self):
        ...
        assert result == 7, (
            "numpy int32 should be converted to Python int"
        )
        assert isinstance(result, int), "Type should be int"
    ...
    def test_numpy_float(self):
        ...
        assert result == pytest.approx(2.5), (
            "numpy float64 should be converted"
        )
    ...
    def test_numpy_bool(self):
        ...
        assert result is True, (
            "numpy bool should convert to Python True"
        )
    ...
    def test_unknown_type_stringified(self):
        ...
        assert isinstance(result, str), (
            "Unknown type should be stringified"
        )
    ...
    def test_item_raises_falls_through(self):
        ...
        assert isinstance(result, str), (
            "Should fall back to str() when .item() fails"
        )
...
class TestNormalizeAttrValue:
    ...
    def test_scalar_int(self):
        ...
        assert result == 42, (
            "Integer scalar should be returned as-is"
        )
    ...
    def test_scalar_string(self):
        ...
        assert result == "units", (
            "String scalar should be returned as-is"
        )
...
class TestReadAttributeValue:
    ...
    def test_read_succeeds(self):
        ...
        assert result == "kg m-2", (
            "Should return the Read() value"
        )
    ...
    def test_read_fails_falls_to_read_as_string(self):
        ...
        assert result == "meters", (
            "Should fall back to ReadAsString"
        )
    ...
    def test_read_fails_falls_to_read_as_double(self):
        ...
        assert result == pytest.approx(0.1), (
            "Should fall back to ReadAsDouble"
        )
    ...
    def test_read_fails_falls_to_read_as_int64(self):
        ...
        result = _read_attribute_value(att)
        assert result == 42, (
            "Should fall back to ReadAsInt64"
        )
    ...
    def test_all_readers_fail(self):
        ...
        result = _read_attribute_value(att)
        assert result is None, (
            "Should return None when all readers fail"
        )
...
class TestReadAttributes:
    ...
    def test_empty_attributes(self):
        ...
        result = _read_attributes(obj)
        assert result == {}, (
            "Should return empty dict for no attributes"
        )
    ...
    def test_attribute_get_name_fails_skipped(self):
        ...
        assert "units" in result, (
            "Good attribute should be present"
        )
        assert len(result) == 1, (
            "Bad attribute should be skipped"
        )
    ...
    def test_attribute_read_fails_gets_normalized_none(self):
        ...
        assert "broken_attr" in result, (
            "Attribute should still be present in the dict"
        )

    def test_read_attribute_value_raises_caught_by_outer_except(
        self, monkeypatch
    ):
        ...
        monkeypatch.setattr(
            utils_mod, "_read_attribute_value", _exploding_read
        )
