...
class TestMeshTopologyInfo:
    ...
    def test_init_required_fields_only(self):
        ...
        assert topo.topology_dimension == 2, f"Expected topology_dimension 2, got {topo.topology_dimension}"
        assert topo.node_x_var == "node_x", f"Expected node_x_var 'node_x', got '{topo.node_x_var}'"
        assert topo.node_y_var == "node_y", f"Expected node_y_var 'node_y', got '{topo.node_y_var}'"
        assert topo.face_node_var is None, f"Expected face_node_var None, got {topo.face_node_var}"
        assert topo.edge_node_var is None, f"Expected edge_node_var None, got {topo.edge_node_var}"
        assert topo.face_edge_var is None, f"Expected face_edge_var None, got {topo.face_edge_var}"
        assert topo.face_face_var is None, f"Expected face_face_var None, got {topo.face_face_var}"
        assert topo.edge_face_var is None, f"Expected edge_face_var None, got {topo.edge_face_var}"
        assert topo.boundary_node_var is None, f"Expected boundary_node_var None, got {topo.boundary_node_var}"
        assert topo.face_x_var is None, f"Expected face_x_var None, got {topo.face_x_var}"
        assert topo.face_y_var is None, f"Expected face_y_var None, got {topo.face_y_var}"
        assert topo.edge_x_var is None, f"Expected edge_x_var None, got {topo.edge_x_var}"
        assert topo.edge_y_var is None, f"Expected edge_y_var None, got {topo.edge_y_var}"
        assert topo.data_variables == {}, f"Expected empty data_variables, got {topo.data_variables}"
    ...
    def test_init_all_fields(self):
        ...
        assert topo.data_variables == data_vars, f"data_variables mismatch: {topo.data_variables}"
    ...
    def test_topology_dimensions(self, topo_dim):
        ...
        """
        topo = MeshTopologyInfo(
            mesh_name="mesh", topology_dimension=topo_dim,
            node_x_var="x", node_y_var="y",
        )
...
class TestMeshVariable:
    ...
    def test_init_with_eager_data(self, face_var_1d):
        ...
        """
        assert face_var_1d.name == "water_level", f"Expected name 'water_level', got '{face_var_1d.name}'"
        assert face_var_1d.location == "face", f"Expected location 'face', got '{face_var_1d.location}'"
        assert face_var_1d.mesh_name == "mesh2d", f"Expected mesh_name 'mesh2d', got '{face_var_1d.mesh_name}'"
        assert face_var_1d.shape == (5,), f"Expected shape (5,), got {face_var_1d.shape}"
        assert face_var_1d.nodata == -999.0, f"Expected nodata -999.0, got {face_var_1d.nodata}"
        assert face_var_1d.units == "m", f"Expected units 'm', got '{face_var_1d.units}'"
    ...
    def test_init_defaults(self):
        ...
        assert var.standard_name is None, f"Expected standard_name None, got {var.standard_name}"
    ...
    def test_data_property_lazy_load(self):
        ...
        var = MeshVariable(
            ...
            shape=(3,), _loader=loader,
        )
        result = var.data
        np.testing.assert_array_equal(result, lazy_data)
        assert call_count == 1, f"Loader should be called once, was called {call_count} times"

        result2 = var.data
        np.testing.assert_array_equal(result2, lazy_data)
        assert call_count == 1, f"Loader should still be called once (cached), was called {call_count} times"
    ...
    def test_n_elements_2d(self, temporal_var):
        ...
        """
        assert temporal_var.n_elements == 5, f"Expected 5, got {temporal_var.n_elements}"
    ...
    def test_n_time_steps_no_time(self, face_var_1d):
        ...
        """
        assert face_var_1d.n_time_steps == 0, f"Expected 0, got {face_var_1d.n_time_steps}"
    ...
    def test_n_time_steps_with_time(self, temporal_var):
        ...
        """
        assert temporal_var.n_time_steps == 3, f"Expected 3, got {temporal_var.n_time_steps}"
    ...
    def test_dtype_from_data(self, face_var_1d):
        ...
        """
        assert face_var_1d.dtype == np.dtype("float64"), f"Expected float64, got {face_var_1d.dtype}"
    ...
    def test_dtype_int_data(self):
        ...
        var = MeshVariable(
            name="v", location="face", mesh_name="m",
            shape=(3,), _data=np.array([1, 2, 3], dtype=np.int32),
        )
    ...
    def test_sel_time_range_valid(self, temporal_var):
        ...
        assert isinstance(result, MeshVariable), f"Expected MeshVariable, got {type(result)}"
        ...
        assert result.mesh_name == temporal_var.mesh_name, "Mesh name should be preserved"
...
class TestUgridMetadata:
    ...
    def test_init_defaults(self):
        ...
        """
        meta = UgridMetadata()
        assert meta.mesh_topologies == (), f"Expected empty tuple, got {meta.mesh_topologies}"
        assert meta.data_variables == {}, f"Expected empty dict, got {meta.data_variables}"
        assert meta.global_attributes == {}, f"Expected empty dict, got {meta.global_attributes}"
    ...
    def test_init_with_topology(self):
        ...
        assert len(meta.mesh_topologies) == 1, f"Expected 1 topology, got {len(meta.mesh_topologies)}"
        assert meta.mesh_topologies[0].mesh_name == "mesh2d", "Topology mesh_name mismatch"
        assert meta.data_variables == {"water_level": "face"}, "data_variables mismatch"
        assert meta.conventions == "CF-1.8 UGRID-1.0", f"conventions mismatch: {meta.conventions}"
