...
class TestWarpTileImage:
    ...
    def test_warped_extent_matches_target(
        ...
        tol = 0.5
        assert abs(west - tw) < tol, (
            f"West {west} should be close to {tw} (tol={tol})"
        )
        assert abs(east - te) < tol, (
            f"East {east} should be close to {te} (tol={tol})"
        )
        assert abs(south - ts) < tol, (
            f"South {south} should be close to {ts} (tol={tol})"
        )
        assert abs(north - tn) < tol, (
            f"North {north} should be close to {tn} (tol={tol})"
        )
    ...
    def test_warp_with_ax_sets_output_resolution(
        ...
        assert warped.shape[1] == 200, (
            f"Expected width ~200, got {warped.shape[1]}"
        )
        assert warped.shape[0] == 100, (
            f"Expected height ~100, got {warped.shape[0]}"
        )
    ...
    def test_warp_without_ax_auto_computes_resolution(
        ...
        assert warped.shape[0] > 0, (
            f"Height should be > 0, got {warped.shape[0]}"
        )
        assert warped.shape[1] > 0, (
            f"Width should be > 0, got {warped.shape[1]}"
        )
    ...
    def test_warp_same_crs_preserves_data(self):
        ...
        assert warped.shape[2] == 4, (
            f"Expected 4 channels, got {warped.shape[2]}"
        )
